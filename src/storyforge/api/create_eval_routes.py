...
class StoryGenerateRequest(BaseModel):
    ...
    debug: bool = Field(
        default=False, description="If true, include retrieval chunks, grounded facts, and attribution debug."
    )
    n_results: int = Field(
        default_factory=_default_n_results,
        ge=1,
        le=20,
...
class StoryGenerateResponse(BaseModel):
    ...
    grounded_extraction: Optional[str] = None
    retrieval_chunks: Optional[list[dict]] = None
    grounded_facts: Optional[list[dict]] = None
    debug_attribution: Optional[dict] = None


class SummaryGenerateRequest(BaseModel):
    story_path: str = Field(..., description="Relative path to the generated story .txt file")
    model_config = ConfigDict(
        json_schema_extra={"example": {"story_path": "Generated_Stories/20260401_120000_generated_story.txt"}}
    )
...
class SummaryEvaluateFileRequest(BaseModel):
    ...
    context_path: Optional[str] = Field(
        default=None, description="Optional: relative path to the original story .txt for comparison"
    )
    save: bool = False
    model_config = ConfigDict(
        json_schema_extra={
...
async def story_generate(request: StoryGenerateRequest):
    try:
        ...
        return StoryGenerateResponse(
            ...
            grounded_extraction=result.get("grounded_extraction"),
            retrieval_chunks=result.get("retrieval_chunks"),
            grounded_facts=result.get("grounded_facts"),
            debug_attribution=result.get("debug_attribution"),
        )
