...
5. **Phase 1 retrieval tuning: stopped here (2026-09).** Re-ran the eval after the A/B; 6 of 30 cases still miss top1. Classified against the true baseline's remaining report:
   ...
   **Offline follow-up (2026-09, no HF credits): three grounding / ingest gaps closed without touching the HF path.** An end-to-end audit found that, independent of the HF outage, the pipeline could *accept* ungrounded stories and that the documented rebuild ingest dropped curated metadata. Fixed with local-only code; clean re-measurement still waits for credits.

   - **No-eval facts gate (P0.3).** `decide_action()`'s no-eval branch accepted on completeness alone -- the exact path that inflated the first 0.88 / later 0.75 accept rates. It now checks `facts_count <= 0` first and returns `RE_RETRIEVE`; incomplete-with-facts still REFINEs, complete-with-facts still ACCEPTs. Iteration records carry `has_eval` so accept rate can be split into scored vs. unscored, and a loop that runs out of iterations with zero facts stops with `max_iterations_no_grounded_facts` instead of plain `max_iterations`.
   - **Local facts provider (P0.2).** The local fallback that failed with `Unterminated string` above now: asks Ollama for schema-constrained JSON (`format=<schema>`, `Local_grounded_facts_json_format`; retried without it if the Ollama build rejects it), appends a strict JSON-only instruction, parses with `salvage_grounded_facts_json()` (fences, prose, bare lists, trailing commas, `<think>`, and truncated output -- keeps every complete fact before the cut), drops facts whose `source_chunk_ids` don't resolve to a retrieved chunk, retries once asking for at most 12 facts, and logs an ERROR with a failure category instead of returning 0 facts silently. `Grounded_facts_provider: "local"` skips the HF call entirely for offline runs. The HF path itself is unchanged.
   - **Rebuild ingest reads story_json (P0.1).** `reset_and_ingest.py` → `ingest_stories_dir()` used to read only `data/stories/*.txt` and hardcode `Author=""` / `Summary=""`, ignoring hand-fixed chunks and section tags (the manifest path, `ingest_manifest.py`, already used them). It now loads `data/story_json/<Title>.json` when present: reviewed chunks + sections + Author/Summary/Display_title when `raw_text` matches the `.txt` (CRLF-insensitive), story-level metadata only plus a warning when stale. `Title` stays the filename stem; the human title goes to `Display_title`. `scripts/validate_chroma_metadata.py` reports what actually landed (coverage %, stories missing Author/Summary, single-section stories, missing Title/`chunk_id`).
   - Tests: `tests/test_agentic_loop.py` (extended), `tests/test_local_facts_fallback.py`, `tests/test_ingest_story_json_metadata.py`, `tests/test_metadata_report.py`; new regression tests fail against the pre-fix code. No generation or retrieval knob changed.

   **Still waiting for HF credits:** the clean `measure_generation_length.py --mode fast --length long` run above. It is the only measurement that can confirm the `/no_think` + 3200-token fix on the scored path; offline runs now report `has_eval: false` iterations honestly instead of counting them as scored accepts. The standing constraint is unchanged: no bigger generation model until that run is clean.
